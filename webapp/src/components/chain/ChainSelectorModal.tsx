...
// Kobalte's Dialog provides focus trap, escape-to-close, body scroll lock,
// ARIA role="dialog" with aria-modal, and restores focus on close — all the
// a11y we previously reimplemented (badly) in a plain <div class=overlay>.
const ChainSelectorModal: Component<ChainSelectorModalProps> = (props) => {
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) props.onClose(); }}>
      <Dialog.Portal>
        <Dialog.Overlay class={s.kobalteOverlay} />
        <Dialog.Content class={s.overlay} onOpenAutoFocus={(e) => {
          ...
          {/* Header */}
          <div class={s.header}>
            <Dialog.CloseButton class={s.back} aria-label="Close chain selector">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M19 12H5" />
                <polyline points="12 19 5 12 12 5" />
              </svg>
            </Dialog.CloseButton>
            <Dialog.Title class={s.title}>Select Chain</Dialog.Title>
            <div class={s.spacer} />
          </div>
          ...
          <div class={s.search}>
            ...
            <Show when={search()}>
              <button
                class={s.clearBtn}
                onClick={() => { setSearch(""); inputRef?.focus(); }}
